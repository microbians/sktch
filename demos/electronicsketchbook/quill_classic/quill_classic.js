...
function drawing(last) {
	...
	d = Math.lerp( stroke.prior().d, d, .1); // Smooth distance
	stroke.last().d = d;

/*
	var strokeWeight = stroke.prior().d;
	p1n = p1.vec(pc).uni().per();
	p1L = p1.add(p1n.mul(+strokeWeight/2));
	p1R = p1.add(p1n.mul(-strokeWeight/2));

	var strokeWeight = stroke.last().d;
	p2n = pc.vec(p2).uni().per();
	p2L = p2.add(p2n.mul(+strokeWeight/2));
	p2R = p2.add(p2n.mul(-strokeWeight/2));

	if (!last) {
		var strokeWeight = Math.lerp(stroke.prior().d, stroke.last().d, Math.distance2(p1,pc)/strokeDistance);
	}  else {
		var strokeWeight = stroke.last().d;
	}

	let ang=Math.abs(Vector.ang3(p1,pc,p2));

	if(ang<Math.PI/2) angd=Math.lerp(2,1, ang/Math.PI/2);
	else angd=1;

	pcn = p1n.add(p2n).uni().mul(angd);
	pcL = pc.add(pcn.mul(+strokeWeight/2));
	pcR = pc.add(pcn.mul(-strokeWeight/2));

	layerStroke.strokeStyle=undefined;
	layerStroke.lineWidth=0;
	layerStroke.fillStyle="black";

	layerStroke.globalAlpha=1
	layerStroke.beginPath();
	layerStroke.moveTo(p1L);
	layerStroke.quadraticCurveTo(pcL,p2L);
	layerStroke.lineTo(p2R);
	layerStroke.quadraticCurveTo(pcR,p1R);
	layerStroke.lineTo(p1L);
	layerStroke.fill();
	...
	layerStroke.strokeStyle="red";
	layerStroke.lineWidth=1;
	layerStroke.fillStyle=undefined;
	layerStroke.globalAlpha=1

	layerStroke.beginPath();
	layerStroke.moveTo(p1L);
	layerStroke.lineTo(pcL);
	layerStroke.lineTo(p2L);
	layerStroke.lineTo(p2R);
	layerStroke.lineTo(pcR);
	layerStroke.lineTo(p1R);
	layerStroke.stroke();
*/
...
// Mirar si lo haogo con RES como lo que hacía en flash
	...
	if (angle<Math.PIHalf) { // IF the angle is closed
